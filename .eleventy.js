...
export default async function (eleventyConfig, options = {}) {
  ...
  if (siteData.extra) {
    if (siteData.extra.keywords && typeof siteData.extra.keywords === "string")
      siteData.extra.keywords = siteData.extra.keywords
        .split(",")
        .map((kw) => kw.trim());

    if (siteData.extra.hwidx && typeof siteData.extra.hwidx === "string")
      siteData.extra.hwidx = siteData.extra.hwidx
        .split(",")
        .map((hw) => hw.trim());
      ...
      siteData.contributors = siteData.contributors.map((contributor) => {
        if (
          ...
          return {
            ...
            email: contributor.email ? contributor.email.trim() : undefined,
          };
        }
  ...
  eleventyConfig.on("eleventy.before", async () => {
    if (__syncChanges) {
      console.warn(
        "[user-doc-base]   Warning: Copying any changed content files from user-doc-base to target project.",
      );
      console.warn(
        "[user-doc-base]            This will overwrite any existing files in the target project!",
      );
    ...
    themeFiles.forEach((file) => {
      ...
      if (!existsSync(targetDir)) {
        console.log(
          `[user-doc-base]   Missing required target directory: ${path.relative(process.cwd(), targetDir)}, creating! ✓`,
        );
      ...
      if (!existsSync(targetPath)) {
        console.log(
          `[user-doc-base]   Copying missing content file from user-doc-base: ${file} ✓`,
        );
        cpSync(sourcePath, targetPath);
      } else {
        if (__syncChanges) {
          if (
            !Buffer.from(readFileSync(sourcePath)).equals(
              Buffer.from(readFileSync(targetPath || "")),
            )
          ) {
            console.log(
              `[user-doc-base]   Copying changed content file from user-doc-base to target project, overwriting: ${file} ✓`,
            );
            cpSync(sourcePath, targetPath);
          }
        } else
          console.log(
            `[user-doc-base]   Content file from user-doc-base already exists in target project, skipping: ${file}`,
          );
      }
    });
  });
  ...
  eleventyConfig.amendLibrary("md", (mdLib) =>
    mdLib.use(emoji).use(markdownItAnchor),
  );
  ...
  eleventyConfig.addShortcode("makecode", function (url) {
    const embedUrl = url.replace(
      ...
      "https://makecode.microbit.org/---codeembed#pub:",
    );
    return [
      `<div style="position:relative;height:calc(600px + 5em);width:100%;overflow:hidden;" class="makecode-embed">`,
      `<iframe style="position:absolute;top:0;left:0;width:100%;height:100%;padding-top: 1rem;" src="${embedUrl}" allowfullscreen="allowfullscreen" frameborder="0" sandbox="allow-scripts allow-same-origin"></iframe>`,
      `</div>`,
